<template>
  <div class="k-line-area bg-1d1d29" style="display: flex;flex-direction: row;">
    <div id="showMaketList" v-show="showMarketlist" @mouseover="showMarketList" @mouseout="closeMarketList" style="position: absolute; left: 18px; background: rgb(19, 24, 30); top: 47px; width: 223px; height: 450px; overflow: auto;">
      <market></market>
    </div>
    <div class="flex column ml3" style="background: #181d25; flex:1;margin-right: 6px;">
      <div class="info-wrap">
        <div @mouseover="showMarketList" @mouseout="closeMarketList" style="display: flex; align-items: center; flex-direction: inherit; background: url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAoAAAAKBAMAAAB/HNKOAAAAG1BMVEUAAADN1uTN1uTN1uTN1uTN1uTN1uTN1uTN1uQDLfgAAAAACHRSTlMAwyRp55YPQu9xBBsAAAApSURBVAjXY0AGER0dHa0MjEBSgIHNo6MlgYFBo6MJKMHeUQCSN2ZAAQAemAg9/FQSTAAAAABJRU5ErkJggg==') 100% center / 10px no-repeat;">{{ rightName }}/{{ leftName }}</div>
        <div>
          <span :class="change<0?'red':'green'">{{ lastPrice }}</span>
          <span>≈ $ {{ (lastPrice)|toFixedN(2) }}</span>
        </div>
        <div>
          <span>{{ $t('tv.change') }}</span>
    ...
    <div style="margin-right: 6px;">
      <exchange style="width: 250px;"></exchange>
    </div>
    <div style="margin-right: 6px;">
      <trade style="width: 250px;"></trade>
    </div>
    <levertrade></levertrade>
  </div>
</template>
<script>
...
import levertrade from "@/view/lever/lever_trade"
...
export default {
  ...
  mounted() {
    ...
    eventBus.$on('updateDaymarkey', msg => {
        if (instance.symbol == msg.symbol) {
          instance.lastPrice = Number(msg.close)
          instance.volumn = msg.volume
          instance.highPrice = Number(msg.high)
          instance.lowPrice = Number(msg.low)
          instance.change = msg.change
        }
        // eventBus.$emit('updateDaymarkey', msg);
    })
