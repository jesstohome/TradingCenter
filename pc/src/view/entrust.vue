  ...
  <div class="entrust">
    ...
    <div class="content" style="min-height: 400px">
      <ul class="list-title fColor2 ft12 flex">
        <li class="fl w20">{{ $t('entrust.ul_li_1') }}</li>
        <li class="fl w20">{{ $t('entrust.ul_li_2') }}</li>
        <li class="fl w20">{{ $t('entrust.ul_li_3') }}</li>
        <li class="fl w20">{{ $t('entrust.ul_li_4') }}</li>
        <li class="fl w20">{{ $t('entrust.ul_li_5') }}</li>
      </ul>
      <div class="containers scroll" v-if="inList.length>0">
        <ul class="list-item fColor1 ft12">
          <li v-for="(item,index) in inList" class="clear">
            <span class="fl w20" :class="{'green':type == 'in'}">{{ 'in' == item.type ? $t("entrust.inbuy") : $t("entrust.outsell") }}</span>
        ...
        <div class="tc pdb20" v-if="loading">
          ...
          <p class="ft12 baseColor">{{ $t('entrust.loading') }}</p>
        </div>
      </div>
      <div class="no_data tc" v-if="inList.length<=0 && !loading">
        <img src="@/assets/images/nodata.png" alt="">
        <p class="fColor2 ft18">{{ $t('entrust.nodata') }}</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      wayList: [
        {title: this.$t('entrust.inbuy'), url: "1"},
        {title: this.$t('entrust.outsell'), url: "2"}
      ],
      inList: []
    };
  },
  ...
  methods: {
    ...
    getData(more) {
      ...
      that.loading = true;
      this.$http({
        url: "/api/coin/list?page=" + page + "&status=1&limit=99999&type="+url,
        method: "get",
        data: {page: page, currency_id: cur_id,status:1,type:url},
        headers: {Authorization: token}
          ...
          .then(res => {
            ...
            if (res.data.type == "ok") {
              ...
              if (more) {
                if (list.length == 0) {
                  that.more = that.$t('entrust.nomore');
                } else {
