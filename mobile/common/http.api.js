...
const install = (Vue, vm) => {
	...
	const user = {
		...
		sendMobileCode: (user_string,area_code) => vm.$u.post("/sms_mobile", {user_string,scene:'register',area_code_id: 1,area_code,lang: 'zh',email_code:''}),
		//邮箱验证码验证
		...
		//邮箱注册
		register: (user_string,code,password,re_password,extension_code,type = 'email',area_code) => vm.$u.post("/user/register", {user_string,code,password,re_password,extension_code,type,area_code}),
		forget: (account,code,password,repassword) => vm.$u.post("/user/forget", {account,code,password,repassword}),
		//登陆
	...
	const mailMessage = {
		getList: () => vm.$u.get("/mailMessage/getList"),
		getdetail: (id) => vm.$u.get("/mailMessage/detail",{id}),
	}
