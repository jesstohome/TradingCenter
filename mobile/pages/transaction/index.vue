<template>
	<view>
		...
		<view class="mx-30 pt-30">
			<view class="d-flex-between-center text-white align-items-center">
				...
				<view class="d-flex align-items-center">
					<u-icon name="star-fill" size="42" v-if="optionalId" @click="delOptional"></u-icon>
					<u-icon name="star" size="42" v-else @click="addOptional"></u-icon>
				</view>
			</view>
			
			<view class="d-flex-between-center pt-30">
				<u-subsection :list="nav" bgColor="rgb(31, 34, 41)" style="height: 70rpx; padding: 6rpx; border-radius: 40rpx; width: 100%;" v-if="showNav"  @change="(index)=>currentNav=index" inactive-color="#fff" :current="currentNav"></u-subsection>
			</view>
		</view>
		<!-- 所有行情列表 -->
		<u-popup v-model="showSelectCoin" mode="top" length="100%" contentBackgroundColor="#333333"
			...
			<view class="px-30 market">
				...
				<u-subsection :list="type" bgColor="#2b2e35" style="height: 60rpx; padding: 6rpx; border-radius: 30rpx; width: 100%;"  @change="(index)=>active=index" inactive-color="rgb(221, 224, 229)" :current="active"></u-subsection>
				...
				<scroll-view scroll-y="true">
					<view class="market-item" v-for="item in quotation.filter(it=>active==1?it.market_from==1:it.market_from!=1)"
		...
		<block v-if="currentNav == 0">
			<view class="footer font-size-28" v-if="symbolQuotation.close">
				<!-- <view class="btn btn1"  @click="$utils.jump(`/pages/transaction/contract?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=0`,'redirectTo')">
					<text class="d-block">{{i18n.buy + '/' + i18n.long}}</text>
				</view>
				<view class="btn btn2"  @click="$utils.jump(`/pages/transaction/contract?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=1`,'redirectTo')">
					<text class="d-block">{{i18n.sell + '/' + i18n.short}}</text>
				</view> -->
				
				<view class="btn_new" style="background-color: rgb(40, 186, 152);" @click="$utils.jump(`/pages/transaction/contract?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=0`,'redirectTo')">
					<text class="d-block font-size-28">{{i18n.buy + '/' + i18n.long}}</text>
				</view>
				<view class="btn_new" style="background-color: rgb(240, 74, 90);" @click="$utils.jump(`/pages/transaction/contract?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=1`,'redirectTo')">
					<text class="d-block font-size-28">{{i18n.sell + '/' + i18n.short}}</text>
				</view>
				<view class="btn_new" style="width: 10%; justify-content: center;" @click="$utils.jump(`/pages/transaction/contract_order?currency_id=${symbolQuotation.currency_id}&legal_id=${symbolQuotation.legal_id}`,'redirectTo')">
					<u-icon name="order"  size="62" class="mr-20" v-if="currentNav == 0 && symbolQuotation.id"></u-icon>
				</view>
			</view>
		</block>
		...
		<block v-if="currentNav == 2">
			<view class="footer font-size-28" v-if="symbolQuotation.close">
				<!-- <view class="btn btn1" @click="$utils.jump(`/pages/transaction/currency?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=0`,'redirectTo')">
					<text class="d-block">{{i18n.buy}}</text>
				</view>
				<view class="btn btn2" @click="$utils.jump(`/pages/transaction/currency?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=1`,'redirectTo')">
					<text class="d-block">{{i18n.sell}}</text>
				</view> -->
				<view class="btn_new" style="background-color: rgb(40, 186, 152);" @click="$utils.jump(`/pages/transaction/currency?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=0`,'redirectTo')">
					<text class="d-block font-size-28">{{i18n.buy}}</text>
				</view>
				<view class="btn_new" style="background-color: rgb(240, 74, 90);" @click="$utils.jump(`/pages/transaction/currency?currency_name=${currency_name}&legal_name=${legal_name}&currency_id=${currency_id}&buy_direction=1`,'redirectTo')">
					<text class="d-block font-size-28">{{i18n.sell}}</text>
				</view>
				<view class="btn_new" style="width: 10%; justify-content: center;" @click="$utils.jump('/pages/transaction/currency_order','redirectTo')">
					<u-icon name="order"  size="62" class="mr-20" v-if="currentNav == 2 && symbolQuotation.id"></u-icon>
				</view>
			</view>
		</block>
		...
		<block v-if="currentNav == 1">
			...
			<view class="footer font-size-22" v-if="symbolQuotation.close">
				...
				<view class="btn_new" style="background-color: rgb(40, 186, 152);" @click="buyDirection=1,showBuyConfirm=true">
					<text class="d-block font-size-28">{{i18n.buyUp}}</text>
				</view>
				<view class="btn_new" style="background-color: rgb(240, 74, 90);" @click="buyDirection=2,showBuyConfirm=true">
					<text class="d-block font-size-28">{{i18n.buyDown}}</text>
				</view>
				<view class="btn_new" style="width: 10%; justify-content: center;" @click="$utils.jump('/pages/transaction/seconds_order?match_id='+symbolQuotation.id,'redirectTo')">
					<u-icon name="order"  size="62" class="mr-20" v-if="currentNav == 1 && symbolQuotation.id"></u-icon>
				</view>
			</view>
		...
		<u-popup v-model="showBuyConfirm" mode="center" length="90%" contentBackgroundColor="rgba(0, 0, 0, .4)"
			 :border-radius="20" >
			<view style="background: white; color: black;">
				<view class="bg_ditu" style="padding: 16px 16px 0px 16px;">
					<text class="d-block font-size-30 font-weight-bold text-center">{{i18n.orderConfirm}}</text>
					...
					<view class="d-flex-between-center py-16">
						...
						<view class="font-weight-bold" :class="buyDirection == 1 ? 'text-success' : 'text-error'" style="display: flex;align-items: center;">
							<image style="width: 50rpx;height: 50rpx;" :src="buyDirection == 1 ? buydown : buyup"></image>
							<text class="ml-10">{{buyDirection == 1 ? i18n.buyUp : i18n.buyDown}}</text>
						</view>
					</view>
					<view class="d-flex-between-center py-16">
					...
					<view class="py-16">
						...
						<view class="d-flex-between-center mt-16" v-if="buyDirection == 1">
							<view v-for="item in timers.slice(0, 3)" class="tag text-center font-size-30 d-block" :class="currentTimer == item.seconds ? 'tag-success py-12' : 'border_4D5563 py-10'" @click="currentTimer=item.seconds,currentRatio=item.profit_ratio,currentTimerObj=item" style="width: 30%;border-radius: 6rpx;">
								<view class="d-flex" style="justify-content: center; align-items: center; padding: 10rpx;">
									<span>{{item.seconds}}s</span>
									<span :class="currentTimer == item.seconds ? '' :'opacity-50'" style="display: flex; height: 100%; width: 1px; color: black; margin: 0 10rpx; justify-content: center;">|</span>
									<span :class="currentTimer == item.seconds ? '' :'opacity-50'" style="display: block; color: black;">{{item.profit_ratio}}%</span>
								</view>
							</view>
						</view>
						<view class="d-flex-between-center mt-16" v-if="buyDirection == 1">
							<view v-for="item in timers.slice(3, 6)" class="tag text-center font-size-30 d-block" :class="currentTimer == item.seconds ? 'tag-success py-12' : 'border_4D5563 py-10'" @click="currentTimer=item.seconds,currentRatio=item.profit_ratio,currentTimerObj=item" style="width: 30%;border-radius: 6rpx;">
								<view class="d-flex" style="justify-content: center; align-items: center; padding: 10rpx;">
									<span>{{item.seconds}}s</span>
									<span :class="currentTimer == item.seconds ? '' :'opacity-50'" style="display: flex; height: 100%; width: 1px; color: black; margin: 0 10rpx; justify-content: center;">|</span>
									<span :class="currentTimer == item.seconds ? '' :'opacity-50'" style="display: block; color: black;">{{item.profit_ratio}}%</span>
						...
						<view class="d-flex-between-center mt-16" v-else-if="buyDirection == 2">
							<view v-for="item in timers.slice(0, 3)" class="tag text-center font-size-30 d-block" :class="currentTimer == item.seconds ? 'tag-error py-12' : 'border_4D5563 py-10'" @click="currentTimer=item.seconds,currentRatio=item.profit_ratio,currentTimerObj=item" style="width: 30%;border-radius: 6rpx;">
								<view class="d-flex" style="justify-content: center; align-items: center; padding: 10rpx;">
									<span>{{item.seconds}}s</span>
									<span :class="currentTimer == item.seconds ? '' :'opacity-50'" style="display: flex; height: 100%; width: 1px; color: black; margin: 0 10rpx; justify-content: center;">|</span>
									<span :class="currentTimer == item.seconds ? '' :'opacity-50'" style="display: block; color: black;">{{item.profit_ratio}}%</span>
						...
						<view class="d-flex-between-center mt-16" v-if="buyDirection == 2">
							<view v-for="item in timers.slice(3, 6)" class="tag text-center font-size-30 d-block" :class="currentTimer == item.seconds ? 'tag-error py-12' : 'border_4D5563 py-10'" @click="currentTimer=item.seconds,currentRatio=item.profit_ratio,currentTimerObj=item" style="width: 30%;border-radius: 6rpx;">
								<view class="d-flex" style="justify-content: center; align-items: center; padding: 10rpx;">
									<span>{{item.seconds}}s</span>
									<span :class="currentTimer == item.seconds ? '' :'opacity-50'" style="display: flex; height: 100%; width: 1px; color: black; margin: 0 10rpx; justify-content: center;">|</span>
									<span :class="currentTimer == item.seconds ? '' :'opacity-50'" style="display: block; color: black;">{{item.profit_ratio}}%</span>
				...
				<view class="py-16" style="padding:16px 16px 0px 16px">
					<text class="opacity-50">{{i18n.number}}</text>
					<input type="number" v-model="number" class="trade-input mt-10 " >
					<view class="mt-22">{{$t("buyCrypto.the_minimum_amount_is")+currentTimerObj.min_amount + ' USDT'}}</view>
					<view class="d-block mt-8 font-size-24 " :class="buyDirection==1?'text-succes':'text-error'">
						<view style="display: flex; align-items: center;">
							<image style="width: 40rpx; height: 40rpx;" class="mr-10" :src="buyDirection==1?sy:syseller"></image>
							{{i18n.estimated+'：'+yqMoney+"USDT"}}
						</view>
					</view>
					<view class="d-block mt-8 font-size-24 opacity-75">
						<view style="display: flex; align-items: center;">
							<image style="width: 40rpx; height: 40rpx;" class="mr-10" :src="yue"></image>
							{{i18n.balance + ':' + Number(microInsurance).toFixed(4) + 'USDT'}}
						</view>
					</view>
				</view>
				
				<view class="d-flex-between-center mt-30" style="padding: 16px 16px 56px 16px;">
					<button class="secondary-button font-size-28 w-48" style="padding: 2px 5px; color: #abb0b7 !important;" @click="showBuyConfirm = false">{{$t("common.cancel")}}</button>
					<button class="w-48 font-size-28" @click="submit" :class="buyDirection == 1 ? 'success-button' : 'error-button'" style="padding: 2px 5px;">{{$t("common.confirm")}}</button>
				</view>
...
<script>
	export default {
		data() {
			return {
				buydown:require("static/img/buy-down.svg"),
				buyup:require("static/img/buy-up.svg"),
				sy:require("static/img/sy.svg"),
				syseller:require("static/img/sy_seller.svg"),
				yue:require("static/img/yue.svg"),
		...
		watch: {
			currentTimer: function(t, a) {
				if (t > 0 && this.number > 0) {
					var e = this.currentRatio / 100
					  , i = this.number * e;
					this.yqMoney = i.toFixed(4)
				} else
					this.yqMoney = 0
			},
			number: function(t, a) {
				if (t > 0 && this.currentTimer > 0) {
					var e = this.currentRatio / 100
					  , i = this.number * e;
					  console.log(1,i,e)
					this.yqMoney = i.toFixed(4)
		...
		methods: {
			...
			getSecondsSeconds(){
				this.$u.api.market.getSecondsSeconds().then(res => {
					...
					this.currentTimerObj = this.timers[0]
					this.currentTimer = this.timers[0].seconds
					this.currentRatio = this.timers[0].profit_ratio
				})
			},
			// 获取所有行情
			getQuotation() {
			...
			getPayable(){
				...
				this.$u.api.market.getPayable().then(res=>{
					const data = res.message.find(item=>item.user_wallet.currency == 3)
					//console.log(data)
...
<style lang="scss" scoped>
	...
	.bg_ditu {
	    background-image: url(../../static/img/ditu.png)!important;
	    background-size: 100% 100%!important;
	}
	.border_4D5563 {
	    border: 1px solid #4d5563;
	}
	.tag-success, .tag-plain-success {
