<template>
	<view class="" style="color: #fff;">
		<u-navbar  :background="{background:'black'}" :custom-back="()=>{$utils.jump('/pages/transaction/index','switchTab')}">
		...
		<view class="px-20 py-30">
			...
			<view class="d-grid mt-20 align-items-center" style="grid-template-columns:2fr 1.2fr ">
				...
				<view class="pr-20 position-relative opacity-90" style="height: 780rpx;">
					...
					<view class="mt-40">
						<input v-model="number" class="trade-input mt-20 " :placeholder="i18n.number">
					</view>
			...
			<u-popup v-model="showConfirmPop" border-radius="10" length="90%">
				<view class="p-30">
					...
					<view class="d-flex-between-center border-bottom-white py-16">
						<text class="opacity-50">{{i18n.multiple}}</text>
						<text class="font-weight-bold">{{multiple}}</text>
					</view>
					<view class="d-flex-between-center border-bottom-white py-16">
						<text class="opacity-50">{{i18n.handlingFee}}</text>
						<text class="font-weight-bold">{{handlingFee}}</text>
					</view>
					<view class="d-flex-between-center border-bottom-white py-16">
						<text class="opacity-50">{{i18n.margin}}</text>
		...
		<!-- 持仓列表 -->
		<u-gap height="16" class="mt-20" bg-color="#3c3c3c"></u-gap>
		<view class="">
			<view class="d-grid py-20 font-size-22 opacity-50" style="grid-template-columns:1fr 1fr 1.4fr .7fr 1.2fr;">
				<text class="d-block text-center" style="color: white;">{{i18n.type}}</text>
				<text class="d-block text-center" style="color: white;">{{i18n.time}}</text>
				<text class="d-block text-center" style="color: white;">{{i18n.price}}</text>
				<text class="d-block text-center" style="color: white;">{{i18n.number}}</text>
				<text class="d-block text-center" style="color: white;">{{i18n.operation}}</text>
			</view>
			<block v-if="positionList.length">
				<view class="d-grid py-20 text-center align-items-center border-bottom" style="grid-template-columns:1fr 1fr 1.4fr .7fr 1.2fr;" v-for="item in positionList">
					<text class="d-block text-center" :class="item.type == 1 ? 'text-success' : 'text-error'" style="color: white;">{{item.type == 1 ? i18n.buy : i18n.sell}}</text>
					<view class="font-size-22 opacity-90" style="color: white;">
						<text class="d-block">{{item.transaction_time.slice(0,10)}}</text>
						<text class="d-block">{{item.transaction_time.slice(10,20)}}</text>
					</view>
					<text class="font-weight-bold opacity-90" style="color: white;">{{Number(item.price).toFixed(4)}}</text>
					<text class="opacity-90" style="color: white;">{{item.share}}</text>
					<view class="w-50 mx-auto" style="color: white;">
...
<style lang="scss" scoped>
	...
	.btn.btn2 {
	    background: linear-gradient(113deg,transparent 15px,#20232a 0) 0 0;
	}
	.buy-type .item::after {
	    display: block;
	    content: "";
