<template>
	<view>
		<u-navbar backIconColor="black" :borderBottom='false' :isFixed="false" :background="{background:'rgb(242, 246, 247)'}" title-color="black" :title="i18n.entrust" :custom-back="backTo"></u-navbar>
		<view class="p-30">
			<u-subsection :list="nav" :current="type" v-if="showNav" @change="changeNav"></u-subsection>
			<view class="mt-30">
				<view class="p-20 box-shadow border-radius-20 mb-20 bg-white text-white" v-for="item in list">
					...
					<view class="d-flex-between-center py-10">
						<text class="opacity-50">{{i18n.price}}</text>
					...
					<view class="d-flex-between-center py-10" >
						<text class="opacity-50">{{i18n.entrustTime}}</text>
						<text class="font-weight-bold  opacity-90">{{item.created_at}}</text>
					</view>
					<view class="d-flex-between-center py-10" >
						<text class="opacity-50">{{i18n.status}}</text>
