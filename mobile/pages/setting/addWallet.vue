<template>
	<view class="text-white">
		<u-navbar backIconColor="black" :borderBottom='false' :isFixed="false" :background="{background:'rgb(242, 246, 247)'}" title-color="black" :title="i18n.addWallet"></u-navbar>
		<view class="mx-30">
			<view class="mt-36 input-item" >
				<text class="d-block font-weight-bold font-size-28">{{i18n.selectCurrency}}</text>
				<input type="text" class="input" @click="showSelectCurrencys()" :placeholder="i18n.plsSelectCurrency" v-model="currency.name">
			</view>
			<view class="mt-36 input-item" >
				<text class="d-block font-weight-bold font-size-28">{{i18n.walletAddress}}</text>
				<input type="text" class="input"  :placeholder="i18n.plsIptAddress" v-model="address">
			</view>
			<view class="mt-36 input-item" >
				<text class="d-block font-weight-bold font-size-28">{{i18n.walletQrcode}}</text>
				<view class="text-center">
					<view class="upload-wrap mt-30 ml-0" @click="uploadImage">
						<u-icon name="plus" size="40" v-if="!qrcode"></u-icon>
						<image class="w-100 h-100" :src="qrcode || retImageUrl" mode="aspectFill" v-else></image>
					</view>
				</view>
			</view>
			<button class="primary-button mt-50" @click="submit">{{$t("common.submit")}}</button>
		</view>
		...
		methods:{
			showSelectCurrencys(){
				...
				this.currencyList.map(el=>{
						itemlistarr.push(el.name)
					})
				uni.showActionSheet({
					itemList: itemlistarr,
					success: (res=> {
						this.currency = this.currencyList[res.tapIndex]
					})
				});
			},
			getCurrencyList(){
