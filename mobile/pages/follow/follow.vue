<template>
	<view class="text-white">
		<u-navbar backIconColor="black" :borderBottom='false' :isFixed="false" :background="{background:'rgb(242, 246, 247)'}" title-color="black" :title="i18n.editCopy">
		...
		<view class="px-30">
			<view class="py-30 ">
				...
				<view class="d-flex mt-20">
					<u-button class="mx-0 mr-16 px-40"  :type="activeCopyType == item.id ? 'primary' : 'black'" v-for="item in copyType" size="medium" @click="activeCopyType = item.id">{{item.name}}</u-button>
				</view>
			...
			<view class="d-flex-between-center py-30 ">
			...
			<view class="mt-30">
				...
				<text class="iconfont icon-checkbox-ok-full text-primary font-size-24" v-else
					@click="hasRead = !hasRead"></text>
				<text class="ml-8 ">
					{{i18n.beforeCopy}}
					<text class="text-primary" @click="jump">《{{i18n.copyAgreement}}》</text>
				</text>
			</view>
		</view>
	</view>
