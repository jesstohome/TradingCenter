<template>
	<view class="pb-50">
		<u-navbar backIconColor="black" :borderBottom='false' :isFixed="false" :background="{background:'none'}" title-color="black" :title="'IEO'+i18n.subscribe">
			<u-icon name="order" color="#000" @click="$utils.jump('/pages/ieo/order')" size="40" slot="right"></u-icon>
		</u-navbar>
		<view class="m-30">
			<view class="p-30 box-shadow border-radius-20 mb-20 bg-black-2 text-white" v-for="item in list">
				<image style="width: 100%;height: 272rpx;" :src="$store.state.baseDomain+item.logo"></image>
